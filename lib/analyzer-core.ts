...
/**
 * Pure (browser + Node) analyzer core. Everything here is string-in / data-out.
 * Node fs and Node path stay in `lib/analyzer.ts`; the same primitives are
 * reused by the client-side folder-picker pathway.
 */
...
export interface FileAnalysisResult extends CoreFileSummary {
  usedBy: string[];
  risk: RiskLevel;
...
/* ------------------------------------------------------------------ */
/* Public per-file summary (no usedBy)                                */
/* ------------------------------------------------------------------ */

export function summarizeFile(relPath: string, source: string): CoreFileSummary {
...
/* ------------------------------------------------------------------ */
/* usedBy resolution against an in-memory file map                    */
/* ------------------------------------------------------------------ */

export interface RepoFileInput {
  /** Repo-root-relative posix path, e.g. "src/pages/ClaimSummaryPage.tsx". */
  path: string;
  source: string;
}

/**
 * Find every file in `allFiles` that depends on `targetRelPath` either through
 * an import that resolves to it (relative or @/) or through JSX/identifier
 * references to its exported component names. Pure string logic.
 */
export function findUsedBy(
  targetRelPath: string,
  exportedComponents: string[],
  allFiles: ReadonlyArray<RepoFileInput>
): string[] {
  const targetBase = stripExt(basename(targetRelPath));
  const targetNoExt = stripExt(targetRelPath);
  const matches: string[] = [];

  for (const file of allFiles) {
    if (file.path === targetRelPath) continue;
    if (referencesTarget(file, targetNoExt, targetBase, exportedComponents)) {
...
function referencesTarget(
  candidate: RepoFileInput,
  targetNoExt: string,
  targetBase: string,
  exportedComponents: string[]
): boolean {
  const candidateDir = posixDirname(candidate.path);
  const sources: string[] = [];

  IMPORT_RE.lastIndex = 0;
  SIDE_EFFECT_IMPORT_RE.lastIndex = 0;
  let m: RegExpExecArray | null;
  while ((m = IMPORT_RE.exec(candidate.source)) !== null) sources.push(m[4]);
  while ((m = SIDE_EFFECT_IMPORT_RE.exec(candidate.source)) !== null) {
    sources.push(m[1]);
  }

  for (const src of sources) {
    let resolvedNoExt: string | null = null;

    if (src.startsWith(".")) {
      // Resolve relative against the candidate's directory, posix-style.
      resolvedNoExt = stripExt(posixResolve(candidateDir, src));
    } else if (src.startsWith("@/")) {
      // `@/foo` aliases to `src/foo` from the repo root in this project.
      resolvedNoExt = stripExt(posixJoin("src", src.slice(2)));
    } else if (src.endsWith(`/${targetBase}`)) {
      // Bare-package convention `…/Foo` mapping to a local file by basename.
      return true;
    }

    if (resolvedNoExt && matchesTarget(resolvedNoExt, targetNoExt)) {
      return true;
    }
  }
  ...
  for (const name of exportedComponents) {
    ...
    if (re.test(candidate.source)) return true;
  }

  return false;
}

function matchesTarget(resolvedNoExt: string, targetNoExt: string): boolean {
  if (resolvedNoExt === targetNoExt) return true;
  // index-file fallback: `import "./foo"` → `foo/index.tsx`
  const targetParent = posixDirname(targetNoExt);
  const targetLeaf = basename(targetNoExt);
  if (targetLeaf === "index" && targetParent === resolvedNoExt) return true;
  return false;
}
...
export function computeAnalyzerRisk(args: {
  usedByCount: number;
  apiCallCount: number;
...
export function buildImpactSummary(args: {
  fileName: string;
  usedByCount: number;
  apiCallCount: number;
  customComponentCount: number;
...
/**
 * Top-level convenience: per-file summary + usedBy lookup against the supplied
 * file set. Used by the client-side folder picker.
 */
export function analyzeFromMemory(
  relPath: string,
  source: string,
  allFiles: ReadonlyArray<RepoFileInput>
): FileAnalysisResult {
  const summary = summarizeFile(relPath, source);
  const usedBy = findUsedBy(relPath, summary.exportedComponents, allFiles);
  const risk = computeAnalyzerRisk({
    usedByCount: usedBy.length,
    apiCallCount: summary.apiCalls.length,
    customComponentCount: summary.customComponents.length,
  });
  ...
  return { ...summary, usedBy, risk, impactSummary };
}
...
/* ------------------------------------------------------------------ */
/* POSIX path helpers (no Node dependency)                            */
/* ------------------------------------------------------------------ */
...
/**
 * Resolve `target` against `base` using POSIX rules. Supports `.` and `..`
 * segments. Both are treated as repo-root-relative strings (no leading slash).
 */
...
function stripExt(p: string): string {
  // Strip the last `.xxx` segment when it's a typical source file extension.
  const base = basename(p);
  const dot = base.lastIndexOf(".");
  if (dot <= 0) return p;
  const ext = base.slice(dot + 1);
  if (!/^[A-Za-z0-9]+$/.test(ext)) return p;
  const dir = posixDirname(p);
  return dir ? `${dir}/${base.slice(0, dot)}` : base.slice(0, dot);
}
